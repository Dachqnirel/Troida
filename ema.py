# ema.py
# =============================================================================
# Подбор оптимальных параметров EMA:
#  - формируем датасет по скользящим окнам (lookback/horizon/step),
#  - размечаем классом = лучшая EMA (Sharpe, затем Sum) на форвардном окне,
#  - обучаем CatBoostClassifier на классах,
#  - выбираем оптимальный период EMA на последнем окне и сравниваем с базовым.
...
def ema_strategy_returns(close: pd.Series, ema: pd.Series, cost_bps: float = 5.0) -> pd.Series:  # Стратегия пересечения цены и EMA
    ...
    for t in range(1, n):
        new = curr
        if curr == 0:
            if close.iloc[t - 1] <= ema_vals[t - 1] and close.iloc[t] > ema_vals[t]:
                new = 1
            elif close.iloc[t - 1] >= ema_vals[t - 1] and close.iloc[t] < ema_vals[t]:
                new = -1
        elif curr == 1:
            if close.iloc[t - 1] >= ema_vals[t - 1] and close.iloc[t] < ema_vals[t]:
                new = 0
...
def make_features_ohlcv_ema(df_window: pd.DataFrame, lookback: int = 1000) -> pd.Series:  # Фичи по последнему окну (цена/объём/диапазоны)
...
def make_grid_ema(periods=None):  # Сетка периодов EMA
    if periods is None:
        periods = [10, 20, 30, 50]
    return [{"period": p} for p in periods]


def make_labeler_ema(grid, cost_bps: float) -> Callable:  # label_fn: лучшая EMA по Sharpe/Sum на форварде
    def label_on_forward(df: pd.DataFrame, start: int, lookback: int, horizon: int):
        X = make_features_ohlcv_ema(df.iloc[start - lookback:start], lookback)
...
def parse_cli_ema_periods(periods_arg: str):  # Парсер сетки периодов из CLI
    if periods_arg and periods_arg.strip():
        return [int(x) for x in periods_arg.split(",")]
    return None


def run_pipeline(args, verbose: bool = True):  # Основной пайплайн EMA
    ...
    periods_arg = _arg("ema_periods", "")
    overrides = _arg("cat_overrides", "")
    plot_flag = bool(_arg("plot", False))

    df = load_csv(csv_path)
    periods = parse_cli_ema_periods(periods_arg)
    grid = make_grid_ema(periods)
    ...
    _log("Рекомендованные параметры EMA:", best_params)

    fw_close = df["close"].iloc[-horizon:]
    ema_rec = compute_ema(fw_close, best_params["period"])
    ret_rec = ema_strategy_returns(fw_close, ema_rec, cost_bps=cost_bps)
    sh_rec, sm_rec = sharpe_and_sum(ret_rec)

    ema_base = compute_ema(fw_close, 20)
    ret_base = ema_strategy_returns(fw_close, ema_base, cost_bps=cost_bps)
...
def main():
    ...
    ap.add_argument("--ema_periods", type=str, default="", help="Сетка периодов EMA, напр.: 10,20,30")
