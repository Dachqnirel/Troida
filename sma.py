# sma.py
# =============================================================================
# Подбор оптимальных параметров SMA:
#  - формируем датасет по скользящим окнам (lookback/horizon/step),
#  - размечаем классом = лучшая SMA (Sharpe, затем Sum) на форвардном окне,
#  - обучаем CatBoostClassifier на классах,
#  - выбираем оптимальный период SMA на последнем окне и сравниваем с базовым.
...
def compute_sma(close: pd.Series, period: int) -> pd.Series:  # Базовый расчёт SMA(period)
    return close.rolling(period, min_periods=1).mean()


def sma_strategy_returns(close: pd.Series, sma: pd.Series, cost_bps: float = 5.0) -> pd.Series:  # Стратегия пересечения цены и SMA (long/short с комиссией)
    ...
    for t in range(1, n):
        new = curr
        if curr == 0:
            if close.iloc[t - 1] <= sma_vals[t - 1] and close.iloc[t] > sma_vals[t]:
                new = 1
            elif close.iloc[t - 1] >= sma_vals[t - 1] and close.iloc[t] < sma_vals[t]:
                new = -1
        elif curr == 1:
            if close.iloc[t - 1] >= sma_vals[t - 1] and close.iloc[t] < sma_vals[t]:
                new = 0
...
def sharpe_and_sum(ret: pd.Series) -> Tuple[float, float]:  # Метрики стратегии: Sharpe и сумма доходности
    r = ret.dropna()
    if len(r) < 3:
...
def make_features_ohlcv_sma(df_window: pd.DataFrame, lookback: int = 1000) -> pd.Series:  # Фичи по последнему окну (статистики цены/объёма/диапазонов)
    ...
    tr = pd.concat(
        ...
        axis=1,
    ).max(axis=1) / (prev_close.replace(0, np.nan) + 1e-12)
...
def make_grid_sma(periods=None):  # Сетка периодов SMA (CLI может переопределить)
    if periods is None:
        periods = [10, 20, 30, 50]
    return [{"period": p} for p in periods]


def make_labeler_sma(grid, cost_bps: float) -> Callable:  # label_fn: по форварду выбираем лучшую SMA (Sharpe, затем Sum)
    def label_on_forward(df: pd.DataFrame, start: int, lookback: int, horizon: int):
        X = make_features_ohlcv_sma(df.iloc[start - lookback:start], lookback)
        fw_close = df["close"].iloc[start:start + horizon]
        scores = []
        for i, g in enumerate(grid):
            sma = compute_sma(fw_close, g["period"]).bfill().ffill()
            strat = sma_strategy_returns(fw_close, sma, cost_bps=cost_bps)
...
def parse_cli_sma_periods(periods_arg: str):  # Парсинг сетки периодов из строки CLI
    if periods_arg and periods_arg.strip():
        return [int(x) for x in periods_arg.split(",")]
    return None


def run_pipeline(args, verbose: bool = True):  # Основной пайплайн SMA: загрузка -> разметка -> CatBoost -> рекомендация
    ...
    periods_arg = _arg("sma_periods", "")
    overrides = _arg("cat_overrides", "")
    plot_flag = bool(_arg("plot", False))

    df = load_csv(csv_path)
    periods = parse_cli_sma_periods(periods_arg)
    grid = make_grid_sma(periods)

    _log("Карта классов:")
    for i, g in enumerate(grid):
        _log(i, g)

    label_fn = make_labeler_sma(grid, cost_bps)
    ...
    X_last = make_features_ohlcv_sma(lb_df, lookback).to_frame().T
    pred_last = int(model.predict(X_last).flatten()[0])
    best_params = grid[pred_last]
    _log("Рекомендованные параметры SMA:", best_params)

    fw_close = df["close"].iloc[-horizon:]
    sma_rec = compute_sma(fw_close, best_params["period"]).bfill().ffill()
    ret_rec = sma_strategy_returns(fw_close, sma_rec, cost_bps=cost_bps)
    sh_rec, sm_rec = sharpe_and_sum(ret_rec)

    sma_base = compute_sma(fw_close, 20).bfill().ffill()
    ret_base = sma_strategy_returns(fw_close, sma_base, cost_bps=cost_bps)
...
def main():
    ...
    ap.add_argument("--sma_periods", type=str, default="", help="Сетка периодов SMA, напр.: 10,20,30")
