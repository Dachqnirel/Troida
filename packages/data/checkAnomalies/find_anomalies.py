...
class CheckAnomalyCandle(ABC):
    ...
    def checkAnomalyInCandles(
        ...
        if mode == "remove":
            ...
            logging.logger.info("[REPORT] Режим: удаление аномалий.")
        elif mode == "fix":
            ...
            logging.logger.info("[REPORT] Режим: исправление аномалий.")
        else:
    ...
    def _annotate_anomalies(
        ...
        logging.logger.info(f"Текущие аномалии: {counts}")
        return df
    ...
    def _fix_anomalies(
        ...
        if bad_ohlc_mask.any():
            ...
            logging.logger.info(f"[REPORT] Исправлено {int(bad_ohlc_mask.sum())} баров с high < low")
        ...
        if zero_move_mask.any():
            ...
            logging.logger.info(f"[REPORT] Исправлено {int(zero_move_mask.sum())} баров с нулевым объёмом и ценовым движением.")
        ...
        if rolling_window and rolling_window > 1:
            ...
            logging.logger.info(f"[REPORT] Исправлено {int(hi_mask.sum())} пиков объёма до {limit:.2f}")
        
        if hi_mask.any():
             logging.logger.info(f"[REPORT Исправлено {int(hi_mask.sum())} пиков объёма с использованием робастного Z-score.")
        ...
        if tr_outlier_mask.any():
            ...
            logging.logger.info(f"[REPORT] Исправлено {int(tr_outlier_mask.sum())} баров с аномальным True Range.")
        ...
        if ret_outlier_mask.any():
            ...
            logging.logger.info(f"[REPORT] Исправлено {int(ret_outlier_mask.sum())} баров с аномальной доходностью.")
        ...
        if "datetime" in fixed_df.columns and fixed_df["datetime"].duplicated().any():
            ...
            logging.logger.info(f"[REPORT] Удалено {dupes} дубликатов datetime.")
