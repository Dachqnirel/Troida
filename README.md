...
Запустить скрипт из директории (```cd ./createReports```) ```createReports``` при помощи вызова команды ```python .createReports.py -f <путь к .yaml конфигурации> -d <путь к директории, куда необходимо загрузить отчёты>```

Для справки может служить команда ```python ./createReports --help```
